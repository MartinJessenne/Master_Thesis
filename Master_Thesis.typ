...
== Algorithms
To ground our analysis, we formally introduce the primary algorithms examined in this thesis, which are implemented in our `optigame` Rust library and python framework for reproducibilty's sake.
...
It is this dependence on the full history of cumulative losses ($L_x^(t-1)$) that causes the "non-forgetfulness" phenomenon, ultimately trapping the algorithm in sub-optimal strategies for extended periods @cai2025fastconvergence.
...
By the linearity of the expected loss, the maximum and minimum over the mixed strategies $y'$ and $x'$ are achieved at pure strategies. Thus, in practice, the duality gap is computed by taking the minimum and maximum components of the vectors $-A^T x$ and $A y$. The duality gap is non-negative and equals zero if and only if $(x, y)$ is a Nash equilibrium.
...
A distinction in the literature is between *uniform* and *universal* convergence rates @cai2025separation. A uniform rate bounds the convergence speed solely as a function of $T$ and the dimensions of the game, independent of specific game properties. A universal rate, however, may depend on game-specific constants, such as the minimum equilibrium probability $delta$. The distinction between these rates is handy when analyzing algorithms like OMWU that suffer from arbitrarily bad dependencies on $delta$.
...
To overcome this, `optigame` is implemented as a Rust core wrapped in a Python interface via PyO3. This architecture allows us to express complex mathematical operations safely in Rust, leveraging its zero-cost abstractions, its strictly typed system to ensure mathematical  logic at compile time, and most importantly, thread-level parallelism. Leveraging concurrency techniques, the algorithm automatically distributes the neighborhood exploration across all available CPU cores. This hybrid approach yields orders-of-magnitude speedups, reducing hours of computation to miliseconds, while allowing the results to be natively analyzed and plotted in interactive Python environments like Jupyter or Marimo.
...
#figure(
  ...
  caption: [
  Benchmark shows a $748 times$ improvement in computation time from switching the computation logic to Rust.
  ],
)
]
#v(2cm)
...
#v(4cm)
#align(center)[
#text("TODO: Reproduce the profile of the strategies in the 2D plan for $A_delta$")
]
#v(4cm)

== Novel Exploration: Simultaneous Boundaries ($A_(lambda, gamma)$)
While the $A_delta$ game exposes OMWU's vulnerability to a single poorly conditioned strategy, real-world min-max optimization problems often involve simultaneous ill-conditioning across all players. To test if this degrades the convergence rates further, we introduce a novel matrix family, $A_(lambda(x), gamma(x))$, utilizing the parameterization technique detailed in Chapter 3.

In this game, we define the target Nash equilibrium explicitly as $x^* = (lambda(x), 1-lambda(x))$ and $y^* = (gamma(x), 1-gamma(x))$, where both $lambda(x)$ and $gamma(x))$ are converging towards arbitrary points of the simplex, with chosen convergence rate. In our analysis, we've use  $lambda(x) = x^2$ and $gamma -> 1/2+x$ as $x -> 0$. By applying our affine normalization to keep the matrix bounded in $[0, 1]$, we ensure the theoretical constraints of OMWU are respected.
...
#figure(
  grid(
    ...
    image("OMWU_x_star_last_iterate_vs_theory.png", width: 100%),
    image("OMWU_y_star_last_iterate_vs_theory.png", width: 100%),
  ),
  caption: [The above plotted runs show that the last iterate convergence is chaotic for OMWU even for Nash Equilibrium without low condition numbers $delta$]
)]
#v(4cm)
...
#figure(
  grid(columns: 2,
  gutter: 1em,
  image("omwu_neigh_expl_plots/Omwu_last_iterate.png", width: 100%),
  image("omwu_neigh_expl_plots/Omwu_random_iterate.png", width: 100%),
  image("omwu_neigh_expl_plots/Omwu_best_iterate.png", width: 100%)
  ),
  caption: [The three above plots show the evolution of the chaotic convergence behavior of OMWU as the $A_(lambda(x), gamma(x))$ matrix converges towards $A_(lambda, gamma)$]
)
]
#v(4cm)
