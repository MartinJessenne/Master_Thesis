...
impl S {
    ...
    pub fn from_projected(v: V) -> Self {
        Self {
            inner: v.simplex_projection(1.0),
        }
    }
    pub fn build(v: V) -> Result<Self, String> {
        ...
        if (sum - 1.0).abs() < 1e-6 && all_positive {
            ...
            return Err(format!(
                "v does not belong on the simplex. Last values : {v:#?}"
            ));
        }
    }
    ...
    pub fn to_uniform_inplace(&mut self) {
        let len = self.inner.len();
        self.inner.mapv_inplace(|_val| 1. / (len as f64));
    }
}
...
mod tests {
    ...
    fn test_simplex_projection_zeros() {
        ...
        assert!(
            (sum - 1.0).abs() < 1e-6,
            "Sum after projection of zeros should be 1.0, was {}",
            sum
        );
        ...
        for &val in splx.iter() {
            assert!(
                (val - 0.25).abs() < 1e-6,
                "Expected equal distribution 0.25, got {}",
                val
            );
        }
    }
    ...
    fn test_splx_build_success() {
        ...
        assert!(
            splx.is_ok(),
            "Build should succeed for valid simplex elements"
        );
    }
    ...
    fn test_splx_build_fail_negative() {
        ...
        assert!(
            splx.is_err(),
            "Build should fail if there is a negative element"
        );
    }
